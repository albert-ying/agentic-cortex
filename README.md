...
- **Structured feedback loop.** Natural language corrections become persistent behavioral rules. Each correction is saved with the rule, the rationale, and the scope. The AI reads all accumulated feedback at session start. This is not RL in the ML sense -- the model weights never change. But the effect is structurally analogous: behavior improves from corrections and stays improved. The prompt evolves; the model stays the same. See [Chapter 8](#chapter-8-the-feedback-loop).
